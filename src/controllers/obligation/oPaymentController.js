...
module.exports = {
  createObligationPayment: async (req, res, next) => {
    try {
      const {
        name,
        category,
      ...
      return res.status(201).json({
        success: true,
        message: "Obligation Payment created successfully",
      });
    } catch (error) {
      console.error(error);
      next(error);
  ...
  updateObligationPayment: async (req, res, next) => {
    try {
      const { id } = req.params;
      const {
        name,
      ...
      await prisma.$transaction(async (prisma) => {
        ...
        if (!existingObligation) {
          throw createError(404, "Obligation Payment not found");
        }
        const findInstitution = await prisma.institution.findUnique({
          where: { id: institutionId, deletedAt: null },
        });
      ...
      return res.status(200).json({
        success: true,
        message: "Obligation Payment updated successfully",
      });
    } catch (error) {
      console.error(error);
      next(error);
  ...
  getAllObligationPayments: async (req, res, next) => {
    try {
      ...
      return res.status(200).json({
        success: true,
        message: "Obligation Payments fetched successfully",
        obligations,
      });
    } catch (error) {
      console.error(error);
  ...
  getObligationPaymentById: async (req, res, next) => {
    try {
      const { id } = req.params;
      ...
      return res.status(200).json({
        success: true,
        message: "Obligation Payment fetched successfully",
        obligation,
      });
    } catch (error) {
      console.error(error);
  ...
  deleteObligationPayment: async (req, res, next) => {
    try {
      const { id } = req.params;
      ...
      return res.status(200).json({
        success: true,
        message: "Obligation Payment deleted successfully",
      });
    } catch (error) {
      console.error(error);
      next(error);
  ...
  getObligationPaymentsByUserId: async (req, res, next) => {
    try {
      const { id } = res.user;
